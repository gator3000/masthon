...
def say_hello(local_client: Client):  # default arument passed to back_tasks
    local_client.post_status(
        "Hello World from the API !"
    )  # use client method `post_status`
...
def post_greetings(local_client: Client):  # default arument passed to back_tasks
    local_client.post_status(
        DEFAULT_MESSAGE.format(asctime())
    )  # use client method `post_status`
...
def post(
    local_client: Client, *ms: str
):  # mean 1 default argument for all cmds (client) and a list of str : a sentence
    """post a status"""
